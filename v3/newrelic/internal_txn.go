	...
	if !txnOpts.SuppressCLM && run.Config.CodeLevelMetrics.Enabled && (run.Config.CodeLevelMetrics.Scope == 0 || (run.Config.CodeLevelMetrics.Scope&TransactionCLM) != 0) {
		...
		if txnOpts.LocationOverride != nil {
			...
			if depth > 0 {
				...
				if txnOpts.IgnoredPrefix == "" {
					txnOpts.IgnoredPrefix = run.Config.CodeLevelMetrics.IgnoredPrefix
					if txnOpts.IgnoredPrefix == "" {
						txnOpts.IgnoredPrefix = defaultAgentProjectRoot
					}
				}
				...
				for moreToRead {
					...
					if !strings.HasPrefix(frame.Function, txnOpts.IgnoredPrefix) {
						break
					}
				}
