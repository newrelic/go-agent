...
func TestTxnTrace(t *testing.T) {
	...
	endBasicSegment(txndata, thread, t1, start.Add(6*time.Second), "t1", nil, nil, "")
	...
	endBasicSegment(txndata, thread, t6, start.Add(10*time.Second), "t6", nil, nil, "")
	endBasicSegment(txndata, thread, t5, start.Add(11*time.Second), "t5", nil, nil, "")
	...
	endBasicSegment(txndata, thread, t4, start.Add(16*time.Second), "t4", nil, nil, "")
...
func TestTxnTraceAsync(t *testing.T) {
	...
	endBasicSegment(txndata, thread1, t1s2, start.Add(4*time.Second), "thread1.segment2", nil, nil, "")
	endBasicSegment(txndata, thread2, t2s1, start.Add(5*time.Second), "thread2.segment1", nil, nil, "")
	...
	endBasicSegment(txndata, thread1, t1s1, start.Add(8*time.Second), "thread1.segment1", nil, nil, "")
	endBasicSegment(txndata, thread3, t3s2, start.Add(9*time.Second), "thread3.segment2", nil, nil, "")
	endBasicSegment(txndata, thread3, t3s1, start.Add(10*time.Second), "thread3.segment1", nil, nil, "")
...
func TestTxnTraceSlowestNodesSaved(t *testing.T) {
	...
	for _, d := range durations {
		...
		endBasicSegment(txndata, thread, s, now, strconv.Itoa(d), nil, nil, "")
	}
...
func TestTxnTraceStackTraceThreshold(t *testing.T) {
	...
	t1 := startSegment(txndata, thread, start.Add(1*time.Second))
	endBasicSegment(txndata, thread, t1, start.Add(2*time.Second), "t1", nil, nil, "")

	// not above stack trace threshold w/out params
	t2 := startSegment(txndata, thread, start.Add(2*time.Second))
	endBasicSegment(txndata, thread, t2, start.Add(4*time.Second), "t2", nil, nil, "")
