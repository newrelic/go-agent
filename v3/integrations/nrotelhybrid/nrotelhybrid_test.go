...
func Test_isTransaction(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Parent is not remote but it is valid. Kind is unspecified.",
			parent: oteltrace.NewSpanContext(oteltrace.SpanContextConfig{
				Remote:  false,
				SpanID:  validSpanID,
				TraceID: validTraceID,
		...
		{
			name: "Parent is not remote but it is not valid. Kind is unspecified.",
			parent: oteltrace.NewSpanContext(oteltrace.SpanContextConfig{
				Remote: false,
			}),
			kind:    oteltrace.SpanKindUnspecified,
		...
		{
			name: "Parent is not remote and is invalid. Kind is internal.",
			parent: oteltrace.NewSpanContext(oteltrace.SpanContextConfig{
				Remote: false,
			}),
			kind:    oteltrace.SpanKindInternal,
		...
		{
			name: "Parent is remote. Kind is server. txnChecker returns true.",
			kind: oteltrace.SpanKindServer,
			parent: oteltrace.NewSpanContext(oteltrace.SpanContextConfig{
				Remote: true,
			}),
		...
		{
			name: "Parent is remote. Kind is server. txnChecker returns false.",
			kind: oteltrace.SpanKindServer,
			parent: oteltrace.NewSpanContext(oteltrace.SpanContextConfig{
				Remote: true,
			}),
		...
		{
			name: "Parent is not remote and is invalid. Kind is client.",
			parent: oteltrace.NewSpanContext(oteltrace.SpanContextConfig{
				Remote: false,
			}),
			kind:    oteltrace.SpanKindClient,
		...
		{
			name: "Parent is not remote but it is valid. Kind is client.",
			parent: oteltrace.NewSpanContext(oteltrace.SpanContextConfig{
				Remote:  false,
				SpanID:  validSpanID,
				TraceID: validTraceID,
		...
		{
			name: "Parent is not remote and is invalid. Kind is producer.",
			parent: oteltrace.NewSpanContext(oteltrace.SpanContextConfig{
				Remote: false,
			}),
			kind:    oteltrace.SpanKindProducer,
		...
		{
			name: "Parent is not remote but it is valid. Kind is producer.",
			parent: oteltrace.NewSpanContext(oteltrace.SpanContextConfig{
				Remote:  false,
				SpanID:  validSpanID,
				TraceID: validTraceID,
		...
		{
			name: "Parent is not remote and is invalid. Kind is consumer. txnCheck returns true.",
			parent: oteltrace.NewSpanContext(oteltrace.SpanContextConfig{
				Remote: false,
			}),
			kind:     oteltrace.SpanKindConsumer,
		...
		{
			name: "Parent is not remote and is invalid. Kind is consumer. txnCheck returns false.",
			parent: oteltrace.NewSpanContext(oteltrace.SpanContextConfig{
				Remote: false,
			}),
			kind:     oteltrace.SpanKindConsumer,
		...
		{
			name: "Parent is not remote but it is valid. Kind is consumer. txnCheck returns true.",
			parent: oteltrace.NewSpanContext(oteltrace.SpanContextConfig{
				Remote:  false,
				SpanID:  validSpanID,
				TraceID: validTraceID,
		...
		{
			name: "Parent is not remote but it is valid. Kind is consumer. txnCheck returns false.",
			parent: oteltrace.NewSpanContext(oteltrace.SpanContextConfig{
				Remote:  false,
				SpanID:  validSpanID,
				TraceID: validTraceID,
		...
		{
			name: "Parent is not remote and is invalid. Kind is not listed.",
			parent: oteltrace.NewSpanContext(oteltrace.SpanContextConfig{
				Remote: false,
			}),
			kind:    8,
		...
		{
			name: "Parent is not remote but it is valid. Kind is not listed.",
			parent: oteltrace.NewSpanContext(oteltrace.SpanContextConfig{
				Remote:  false,
				SpanID:  validSpanID,
				TraceID: validTraceID,
...
func Test_nrotelhybridProcessor(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "Basic txn. Defaults to SpanKind.INTERNAL",
		...
		{
			name:     "Basic txn. Set to SpanKind.INTERNAL",
			spanName: "transaction-a",
			fn: func(ctx context.Context, spanName string, tp *trace.TracerProvider) oteltrace.Span {
				tracer := otel.Tracer("test")
		...
		{
			name:     "Basic txn. Set to SpanKind.CLIENT",
			spanName: "transaction-a",
			fn: func(ctx context.Context, spanName string, tp *trace.TracerProvider) oteltrace.Span {
				tracer := otel.Tracer("test")
		...
		{
			name:     "Basic txn. Set to SpanKind.UNSPECIFIED",
			spanName: "transaction-a",
			fn: func(ctx context.Context, spanName string, tp *trace.TracerProvider) oteltrace.Span {
				tracer := otel.Tracer("test")
		...
		{
			name:     "Basic segment when txn exists. Defaults to SpanKind.INTERNAL",
			spanName: "transaction-a",
			fn: func(ctx context.Context, spanName string, tp *trace.TracerProvider) oteltrace.Span {
				tracer := otel.Tracer("test")
				_, span := tracer.Start(ctx, spanName)
			...
			existingTxn:         true,
			expectTxnMapLen:     1,
			expectSegmentMapLen: 1,
		},
		{
			name:     "Basic segment when txn exists. Set to SpanKind.INTERNAL",
			spanName: "transaction-a",
			fn: func(ctx context.Context, spanName string, tp *trace.TracerProvider) oteltrace.Span {
				tracer := otel.Tracer("test")
				_, span := tracer.Start(ctx, spanName, oteltrace.WithSpanKind(oteltrace.SpanKindInternal))
		...
		{
			name:     "Basic segment when txn exists. Set to SpanKind.SERVER",
			spanName: "transaction-a",
			fn: func(ctx context.Context, spanName string, tp *trace.TracerProvider) oteltrace.Span {
				tracer := otel.Tracer("test")
				_, span := tracer.Start(ctx, spanName, oteltrace.WithSpanKind(oteltrace.SpanKindServer))
		...
		{
			name:     "Basic segment when txn exists. Set to SpanKind.CLIENT",
			spanName: "transaction-a",
			fn: func(ctx context.Context, spanName string, tp *trace.TracerProvider) oteltrace.Span {
				tracer := otel.Tracer("test")
				_, span := tracer.Start(ctx, spanName, oteltrace.WithSpanKind(oteltrace.SpanKindClient))
		...
		{
			name:     "Basic segment when txn exists. Set to SpanKind.CONSUMER",
			spanName: "transaction-a",
			fn: func(ctx context.Context, spanName string, tp *trace.TracerProvider) oteltrace.Span {
				tracer := otel.Tracer("test")
				_, span := tracer.Start(ctx, spanName, oteltrace.WithSpanKind(oteltrace.SpanKindConsumer))
		...
		{
			name:     "Basic segment when txn exists. Set to SpanKind.UNSPECIFIED",
			spanName: "transaction-a",
			fn: func(ctx context.Context, spanName string, tp *trace.TracerProvider) oteltrace.Span {
				tracer := otel.Tracer("test")
				_, span := tracer.Start(ctx, spanName, oteltrace.WithSpanKind(oteltrace.SpanKindUnspecified))
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.existingTxn {
				tracer := otel.Tracer("test")
				var transaction oteltrace.Span
				ctx, transaction = tracer.Start(ctx, "outermost-span") // start a generic transaction
				defer transaction.End()
			}

			span := tt.fn(ctx, tt.spanName, tp)
